...
class RadarService {
  ...
  // Decline (reject) a radar invite (Participant Side).
  Future<void> declineRadar(String radarId) async {
    final userId = _supabase.auth.currentUser?.id;
    if (userId == null) throw Exception("User belum login");
    ...
    try {
      ...
      await _insertRadarLog(
       ...
       _logPostgrestError(
        tag: '[RADAR DECLINE ERROR]',
        error: e,
        stackTrace: st,
        payload: {'radar_id': radarId},
      );
      throw Exception("Gagal menolak undangan");
    }
  }
