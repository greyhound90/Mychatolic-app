  ...
  Future<void> _performReverseLookup() async {
     try {
        ...
        if (rawParish.isNotEmpty) {
           ...
           final byId = await _supabase.from('churches').select('id, name').eq('id', rawParish).maybeSingle();
           ...
           if (byId != null) {
              ...
              final byName = await _supabase.from('churches').select('id, name').eq('name', rawParish).maybeSingle();
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      final updates = {
        ...
        'parish': _selectedParishId ?? _parishController.text.trim(), // Prefer ID update, fallback to text
