...
class _CreatePostScreenState extends State<CreatePostScreen> {
  ...
  String _statusMessage =
      "Kirim"; // Untuk feedback visual (Mengompresi/Mengirim)
  ...
  Future<void> _sendPost() async {
    ...
    try {
      ...
      String? cleanId(String? id) =>
          (id == null || id.trim().isEmpty || id == 'null') ? null : id;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "Gagal Posting (Database): ${e.message}\nDetails: ${e.details ?? ''}",
            ),
      ...
      if (mounted && _isSending) {
        // Only reset if still on screen and not success popped
        setState(() {
          _isSending = false;
